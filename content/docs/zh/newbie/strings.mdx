...
<Callout type="warn">
**释放锁要小心！** 不能简单 `DEL lock:order:42`，因为你的锁可能已超时过期、被别人重新持有，这时你的 `DEL` 会误删别人的锁。正确做法是「比对值再删除」，这必须原子完成——用一段 Lua 脚本（先 `GET` 比对再 `DEL`），详见[脚本与函数](/docs/newbie/scripting-and-functions)。
</Callout>
...
继续阅读 → [Hash：存对象的最佳选择](/docs/newbie/hashes)
