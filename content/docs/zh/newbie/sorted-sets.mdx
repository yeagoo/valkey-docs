...
<Callout type="warn">
「取出」和「删除」之间存在竞态：多个消费者可能同时捞到同一个任务。生产环境要把「取 + 删」放进一段 Lua 脚本里原子执行，详见[脚本与函数](/docs/newbie/scripting-and-functions)。
</Callout>
...
继续阅读 → [Stream：append-only 日志与消费组](/docs/newbie/streams)
